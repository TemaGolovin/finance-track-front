...
| Высокий | **Полное удаление аккаунта** (по инициативе пользователя) | Бэкенд: удаление пользователя и связанных записей (каскады Prisma / явные шаги), инвалидция refresh, при необходимости политика для групповых данных (что происходит с операциями в совместных группах). Фронт: экран или блок в профиле с явным подтверждением (повтор пароля / фраза). Юридически: ожидание пользователя и право на удаление персональных данных; тексты политики/соглашения при необходимости уточнить. Детали — §5 приоритетного бэклога. |
...
### 5. Полное удаление аккаунта пользователем

**Что:** Самообслуживаемое необратимое (или с явной политикой мягкого удаления) удаление учётной записи и персональных данных по действию пользователя в интерфейсе.

**Зачем:** Доверие, соответствие ожиданиям по обработке ПДн (в т.ч. 152-ФЗ — прекращение обработки / удаление при отзыве согласия, если иное не вытекает из закона), снижение обращений в поддержку.

**Состав работ:**

| Слой | Содержание |
|------|------------|
| **Бэкенд** | Эндпоинт (например `DELETE` или `POST` с подтверждением) под JWT; транзакция: удаление `User` и зависимостей по схеме Prisma (`onDelete`, каскады), либо поэтапная чистка; отзыв всех refresh-токенов / сессий; решение по **группам** (удалить членство и персональные операции; для чужих групповых операций — оставить агрегат без ПДн или переназначить — зафиксировать продуктово). |
| **Фронт** | Раздел в профиле / настройках: предупреждение о последствиях, подтверждение (пароль), при успехе — выход и редирект на главную или логин. |
| **Юридика** | При необходимости доп. абзац в политике ПДн / соглашении о сроках и полноте удаления. |

**Зависимости:** Актуальная схема БД (`finance-track-back`, `prisma/schema.prisma`); согласованность с email-токенами и почтовыми задачами после удаления.

**Сложность:** M (выше, если неочевидная модель данных по группам и совместным операциям).
