...
| **Полное удаление аккаунта** | **Бэкенд (`finance-track-back`):** `POST /auth/delete-account` под JWT, в теле `{ password }`; проверка пароля (`bcrypt`), транзакция Prisma: `personalCategoryMap`, персональные `operation`/`category`, приглашения (`invitation`), членство в группах (`userRelationGroupUser`); группы, где пользователь был создателем — передача `creatorId` первому оставшемуся участнику или удаление пустой группы; затем `refreshToken`, `emailToken`, `user`. **Фронт:** фича `src/feature/account-delete/` (`AccountDeleteSection` в `profile-overview`), мутация `useDeleteAccount` → очищает кэш `auth.me` и сессий; i18n ru/en. В политике/соглашении уже есть общие формулировки про удаление учётной записи. |
...
| — | **Полное удаление аккаунта** (по инициативе пользователя) | **Сделано** — см. таблицу «Сделано» (`POST /auth/delete-account`, блок в профиле с подтверждением паролем; политика по создаваемым группам — см. `AuthService.deleteAccount`). |
...
### 5. ~~Полное удаление аккаунта пользователем~~ — **Сделано**

> Реализовано: `POST /auth/delete-account`, транзакционная чистка данных и сессий, политика по группам создателя — см. таблицу «Сделано» выше и `AuthService.deleteAccount` в `finance-track-back`.

**Что было целью:** Самообслуживаемое необратимое удаление учётной записи и персональных данных по действию пользователя в интерфейсе.

**Зачем:** Доверие, соответствие ожиданиям по обработке ПДн (в т.ч. 152-ФЗ), снижение обращений в поддержку.

**Итог реализации (кратко):**

| Слой | Содержание |
|------|------------|
| **Бэкенд** | `POST` с паролем под JWT; транзакция с явным порядком удаления связанных сущностей; refresh и email-токены; для групп, созданных пользователем — смена создателя на другого участника или удаление группы, если участников не осталось. |
| **Фронт** | Блок в профиле (`AccountDeleteSection`), drawer с вводом пароля, toasts, сброс кэша после успеха. |
| **Юридика** | Общие пункты в действующих текстах политики и соглашения про удаление учётной записи; при необходимости позже уточнить сроки/полноту отдельным абзацем. |
...
**Смысл:** Транзакционная почта (§1) — **сделана**, база для смежных сценариев auth (OAuth, magic link) готова. Базовый профиль + смена email **сделаны**; удаление аккаунта (§5) — **сделано**. Следующие естественные шаги: OAuth/Passkeys (§3) и PWA/push (§4); фильтры относительно автономны. **Роли в группе** — отдельная тема из таблицы идей, на схеме без жёстких стрелок.
...
Бэкенд: схема БД — репозиторий `finance-track-back`, файл `prisma/schema.prisma`; авторизация и удаление аккаунта — `src/auth/` (NestJS), эндпоинт `POST delete-account`.



Заметки которые потом нужно оформить, когда это будешь читать оформи в задачи пожалуйста:
- в профиле, если пользователь не подтвердил email должна быть повторная возможность для отправки письма для подтверждения
- везде где нет empty-экранов сделать их, например, для списка групп, если там пусто, то должна быть надпись что групп нет и кнопка для создания первой группы
- не работает инвалидация данных после создания группы и других сущностей
- сделать маски для ценовых инпутов(разделение тысяч пробелом)
- сделать скелетоны при загрузке данных
- баг при изменении категории, не меняется цвет и иконка
- нет обновления refresh access для PUT POST PATCH запросов
- при редактировании группы отдает 404 почему то
- 